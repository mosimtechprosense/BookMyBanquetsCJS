      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
        {category.map((cat, index) => (
          ...
          onClick={() => handleCategory(cat)}
            key={index}
            className={`flex items-center justify-between rounded-lg overflow-hidden ${cat.colour} shadow hover:shadow-lg transition-all duration-300 h-[100px] cursor-pointer`} 
          >
            {/* Left Side: Text */}
